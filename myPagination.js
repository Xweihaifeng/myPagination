...
Page.prototype = {
    init: function init() {
        ...
        that.getPages().forEach(function(item) {
            ...
            if (item == that.curPage) { li.className = 'active'; } else {
                li.onclick = function() {
        ...
        that.nextPage();
        that.finalPage();
        if (that.showSkipInputFlag) { that.showSkipInput(); }
        ...
        if (this.curPage <= this.pageTotal) {
            if (this.curPage < this.pageSize) { var i = Math.min(this.pageSize, this.pageTotal); while (i) { pag.unshift(i--); } } else {
                var middle = this.curPage - Math.floor(this.pageSize / 2),
                    i = this.pageSize;
                if (middle > this.pageTotal - this.pageSize) { middle = this.pageTotal - this.pageSize + 1; }
                while (i--) { pag.push(middle++); }
            }
        } else { console.error('当前页数不能大于总页数'); }
